...
quantiles_cm <- quantile(d_cm_data[,1])[-1]
quantiles_nutrition <- quantile(nutrition_data[,1])[-1]
quantiles_satt <- quantile(satt_data[,1])[-1]
quantiles_educ <- quantile(education_data[,1])[-1]
quantiles_elct <- quantile(electricity_data[,1])[-1]
quantiles_water <- quantile(water_data[,1])[-1]
quantiles_sani <- quantile(sani_data[,1])[-1]
quantiles_housing <- quantile(housing_data[,1])[-1]
quantiles_cooking <- quantile(cooking_fuel_data[,1])[-1]
quantiles_assets <- quantile(assets_data[,1])[-1]
...
quantiles_df_comp$`25%` <- unlist(quantiles_df[1,])
quantiles_df_comp$`50%` <- unlist(quantiles_df[2,])
quantiles_df_comp$`75%` <- unlist(quantiles_df[3,])
quantiles_df_comp$`100%` <- unlist(quantiles_df[4,])
quantiles_df_comp$Indicator <- factor(quantiles_df_comp$Indicator, levels=rev(variables_full))

ggplot(data = quantiles_df_comp) +
  ...
  ggtitle("Distributions of Deprivation Rates of all Multidimensional Poverty Indicators") +
  theme_minimal() +
...
plot_MRD_heatmap <- ggplot(MRD_df_long, aes(quantile, Variable, fill= value)) + 
  ...
  geom_text(aes(label=round(value,2)), colour = "white") +
  scale_fill_viridis(discrete=FALSE) + 
  labs(title = "Mean Average Deviation of Each Model in Different Deprivation Quantiles") + 
  xlab("Quantiles (low to high)") + ylab("") +
  theme_minimal() + 
  theme(
    legend.position = "none",
...
plot_MRD_marginal_no_heatmap <- ggplot(MRD_marginalno_df_long, aes(class, Variable, fill= value)) + 
  ...
  geom_text(aes(label=round(value,2)), colour = "white") +
  scale_fill_viridis(discrete=FALSE) + 
  labs(title = "Mean Average Deviation of Each Model for Non-Deprived & \nMarginally Deprived (10% and less) clusters") + 
  xlab("") + ylab("") +
  theme_minimal() + 
  theme(
    legend.position = "none",
...
plot_demographic_disaggregation <- function(pred.1 = pred_1,
                                            ...
                                            ytitle = "Cooking Fuel Deprivation Rate Young 18-34"){
  ...
  print(plot)
  ggsave(paste0("../Data Exports/Figures/Disaggregation ", title,".png"), width = 8, height = 8)
}
...
df_cor_rel_religion_castes %>% arrange(Religion_Caste)
...
religion_caste <- c("Schedule_Caste", "Schedule_Tribe", "OBC", "Unclassified", "Hindu", "Muslim", "Christian", "Sikh")
df_deprivation_religion_caste <- df_deprivation_religion_caste[,-1]
df_deprivation_religion_caste$Religion_Caste <- factor(religion_caste, levels = rev(religion_caste))
...
plot_deprivation_religion_caste_heatmap <- ggplot(df_deprivation_religion_caste, aes(x = Indicator, y = Religion_Caste, fill= Deprived)) + 
  geom_tile(colour = "white") +
  geom_text(aes(label=paste0(format(round(Deprived, 2), nsmall = 0), "%")), colour = "white") +
  scale_fill_viridis(discrete=FALSE) + 
  scale_x_discrete(position = "top") +
  labs(title = "Deprivation Rates across Indicators, Religions and Castes") + 
  xlab("") + ylab("") +
  theme_minimal() + 
  theme(
    legend.position = "none",
...
plot_deprivation_religion_caste_heatmap
